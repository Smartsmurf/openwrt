			   ...
			   unsigned int r)
{
	...
	while (c != r) {
		...
		dma_sync_single_for_cpu(geth->dev, mapping,
					word0.bits.buffer_size,
					DMA_TO_DEVICE);
...
static int gemini_ethernet_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(map)) {
		...
		ret = regmap_read(map, 0x30, &val);
		if (ret) {
			dev_err(dev, "unable to read register\n");
			ret = -ENODEV;
			/* goto out_unprep_clk; */
		}
		dev_err(dev, "regmap[0x30] = 0x%08x\n", val );
	}
